...
def lftensor2lfnp(lftensor):
    ...
    '''
    N, V, V, C, H, W = lftensor.shape
    lftensor = lftensor.reshape(N, V*V, C, H, W)
...
def get_paths(save_path, step, N):
    ...
    img_paths = []
    for k in range(N):
        lf_paths.append(os.path.join(save_path, f'pred_lf_{step*N+k:02d}.mp4'))
        img_paths.append(os.path.join(save_path, f'inp_img_{step*N+k:02d}.jpg'))
